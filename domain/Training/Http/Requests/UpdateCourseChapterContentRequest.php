...
class UpdateCourseChapterContentRequest extends FormRequestCustomizer
{
    /**
    ...
    {
        $key = resolve_key_to_use(CourseChapterContent::class, $this ->route('id'));

        return [
            'title'             => 'required|string|max:255',
            'slug'              => [
                'string', 'max:255', Rule::unique('course_chapter_contents', 'slug') ->ignore($this ->route('id'), $key)
            ],
